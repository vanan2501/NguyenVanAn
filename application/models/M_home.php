...
class M_Home extends CI_Model
{
	...
	public function load_slide_newest()
	{
		...
		$this->db->select('id_cs,thumb_cs,ten_cs,gia_cs')->where('course.id_user = user.id_user')->order_by('id_cs', 'DESC')->limit(7);
	...
	public function load_slide_price()
	{
		...
		$this->db->select('id_cs,thumb_cs,ten_cs,gia_cs')->where('course.id_user = user.id_user')->order_by('gia_cs', 'DESC')->limit(7);
		$query = $this->db->get('course, user');
		return $query->result_array();
	}
	public function load_slide_rand()
	{
		...
		$this->db->select('id_cs,thumb_cs,ten_cs,gia_cs')->where('course.id_user = user.id_user')->order_by('id_cs', 'RANDOM')->limit(7);
		$query = $this->db->get('course, user');
		return $query->result_array();
	}
}
