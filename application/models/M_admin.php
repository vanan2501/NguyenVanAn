...
class M_Admin extends CI_Model
{
	...
	public function qlgv(){
		$per = 2;
		$query = $this->db->get_where('user',array('permission_user' => $per));
		return $query->result_array();
	}
    ...
    public function employeeList() {
   	...
   	public function order_chart(){
   		$data['chart']=$this->db->select( (MONTH(date_own)) , COUNT(id_own)-> from ('own')->qb_groupby('date_own'));
   		$this ->load->view('chart_order',$data);
   	}
}